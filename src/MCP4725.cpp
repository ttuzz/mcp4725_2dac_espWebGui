...
// Voltage functions
int MCP4725::setVoltage(float voltage, float vref)
{
  if (!isValidVoltage(voltage, vref)) return MCP4725_VOLTAGE_ERROR;
  uint16_t dacValue = _voltageToDAC(voltage, vref);
  return setValue(dacValue);
}

float MCP4725::getVoltage(float vref)
{
  return _dacToVoltage(_lastValue, vref);
}

int MCP4725::setMilliVolt(uint16_t milliVolt, float vref)
{
  float voltage = milliVolt / 1000.0;
  return setVoltage(voltage, vref);
}

uint16_t MCP4725::getMilliVolt(float vref)
{
  return (uint16_t)(getVoltage(vref) * 1000);
}

// Error description function
String MCP4725::getErrorDescription(int errorCode)
{
  switch(errorCode) {
    case MCP4725_OK:              return "OK";
    case MCP4725_VALUE_ERROR:     return "Invalid DAC value (0-4095)";
    case MCP4725_REG_ERROR:       return "Register error";
    case MCP4725_NOT_CONNECTED:   return "Device not connected";
    case MCP4725_VOLTAGE_ERROR:   return "Invalid voltage value";
    default:                      return "Unknown error";
  }
}

// Impedance mode description function
String MCP4725::getImpedanceMode()
{
  uint8_t mode = readPowerDownModeDAC();
  switch(mode) {
    case MCP4725_PDMODE_NORMAL: return "Normal (~1Ω)";
    case MCP4725_PDMODE_1K:     return "1kΩ Pull-Down";
    case MCP4725_PDMODE_100K:   return "100kΩ Pull-Down";
    case MCP4725_PDMODE_500K:   return "500kΩ Pull-Down";
    default:                    return "Unknown";
  }
}

// Utility functions
uint16_t MCP4725::_voltageToDAC(float voltage, float vref)
{
  if (voltage < 0) voltage = 0;
  if (voltage > vref) voltage = vref;
  return (uint16_t)((voltage / vref) * MCP4725_MAXVALUE);
}

float MCP4725::_dacToVoltage(uint16_t dacValue, float vref)
{
  return (dacValue / (float)MCP4725_MAXVALUE) * vref;
}
